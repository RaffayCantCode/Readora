...
export async function searchGoogleBooks(query: BookSearchQuery): Promise<{ books: ProviderBook[]; total: number }> {
  const params = new URLSearchParams({ q: queryValue(query), startIndex: String((query.page - 1) * query.limit), maxResults: String(Math.min(query.limit, 20)), printType: "books" });
